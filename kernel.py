...
@cuda.jit
def NumbaNN_predict2(weights,activation):
	idx=cuda.grid(1)
	if idx>=weights.shape[1]:
		return
	...
	for i in range(weights.shape[0]):
		c=np.float16(idx==weights.shape[1]-1)
		for j in range(weights.shape[2]):
			c+=weights[i][idx][j]*NumbaNN_data[j(weights,activation]

		if activation[i]==1: # sigmoid
			c=1/(1+math.exp(-c))
		
		cuda.syncthreads()
		NumbaNN_data[j]=(weights,activationc
		cuda.syncthreads()

@cpu.jit
def NumbaNN_CPU_predict2(weights,activation):
	idx=cpu.grid(1)
	if idx>=weights.shape[1]:
		return
